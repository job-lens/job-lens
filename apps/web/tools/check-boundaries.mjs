...
const rules = JSON.parse(
  fs.readFileSync(path.resolve(import.meta.dirname, '../architecture.generated.json')),
);
export function check(source, file) {
  ...
  function inspect(specifier) {
    ...
    if (file.startsWith('shared/') && /^(app|features)\//.test(target))
      errors.push(`${file} -> ${target}: shared boundary`);
    if (own && target.startsWith('app/')) errors.push(`${file} -> ${target}: feature imports app`);
    if (
      own &&
      other &&
      other !== own &&
      (!rules[own]?.depends_on.includes(other) ||
        !new RegExp(`^features/${other}/public$`).test(target))
    )
      errors.push(`${file} -> ${target}: private feature import`);
  }
  function visit(node) {
    if (
    ...
    if (
      ...
      errors.push(`${file}: CommonJS imports are not allowed in the ESM feature graph`);
    if (ts.isCallExpression(node) && node.expression.kind === ts.SyntaxKind.ImportKeyword) {
      if (!node.arguments[0] || !ts.isStringLiteral(node.arguments[0]))
        errors.push(`${file}: nonliteral dynamic import`);
...
const errors = walk(root)
  .filter(f => /\.[jt]sx?$/.test(f) && !/\.d\.ts$|\.test\.|test\//.test(f))
  .flatMap(f =>
    check(fs.readFileSync(f, 'utf8'), path.relative(root, f).split(path.sep).join('/')),
  );
