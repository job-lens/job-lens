...
function renderGate() {
  const router = createMemoryRouter([
    ...
    {element:<SessionGate role="counselor" />,children:[{path:'/protected',element:<h1>受保护内容</h1>}]},
  ],{initialEntries:['/protected']});
  const client = new QueryClient({defaultOptions:{queries:{retry:false}}});
  render(<QueryClientProvider client={client}><RouterProvider router={router} /></QueryClientProvider>);
}
it('does not expose counselor content to learners', async () => {
  server.use(http.get('*/api/v1/me',()=>HttpResponse.json({id:'x',display_name:'测试',roles:['learner']})));
  renderGate(); expect(await screen.findByRole('alert')).toHaveTextContent('无权访问');
