...
it('reports unsupported vibration without breaking training', () => {
  ...
  expect(webPlatform.vibrate({...preferences,quiet_mode:false})).toEqual({ok:false,reason:'unsupported'});
  vi.unstubAllGlobals();
});
it('supports cancelling image selection', async () => {
  const abort = new AbortController(); abort.abort();
  expect(await webPlatform.pickImage(false,abort.signal)).toEqual({ok:false,reason:'cancelled'});
});
