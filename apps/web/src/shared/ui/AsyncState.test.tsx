...
it('offers an accessible retry action', async () => {
  const retry = vi.fn(); render(<ErrorPanel message="连接失败" retry={retry} />);
...
it('announces loading', () => { render(<LoadingState />); expect(screen.getByRole('status')).toHaveTextContent('正在读取'); });
