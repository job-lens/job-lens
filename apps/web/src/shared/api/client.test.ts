...
describe('contract transport', () => {
  it('preserves concurrency headers', () => {
    expect(commandHeaders(7, 'operation-1234567')).toEqual({
      'If-Match': '"7"',
  ...
  it('maps error responses without treating 412 as success', () => {
    expect(() =>
      unwrap({
        response: new Response('', { status: 412 }),
        error: { code: 'VERSION_CONFLICT', title: '请刷新', trace_id: 'trace' },
      }),
    ).toThrow(ApiError);
  });
  it('injects CSRF and keeps credentials out of URL', async () => {
    ...
    await client.POST('/auth/logout', {
      baseUrl: 'https://job-lens.invalid/api/v1',
      params: { header: { 'X-CSRF-Token': 'csrf-token-for-current-session' } },
    });
