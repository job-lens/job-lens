...
export class ApiError extends Error {
  constructor(readonly status: number, readonly code: string, message: string, readonly traceId?: string) {
    super(message);
    this.name = 'ApiError';
  }
}
...
export function createApiClient(fetcher?: typeof fetch) {
  ...
  return { client, setCsrfToken: (value?: string) => { csrfToken = value; } };
}
...
export function unwrap<T>(result: { data?: T; error?: unknown; response: Response }): T {
  ...
  throw new ApiError(result.response.status, typeof error?.code === 'string' ? error.code : 'REQUEST_FAILED',
    typeof error?.title === 'string' ? error.title : '请求未完成，请重试',
    typeof error?.trace_id === 'string' ? error.trace_id : undefined);
}

export function commandHeaders(version: number, key: string): { 'If-Match': string; 'Idempotency-Key': string } {
  if (!Number.isSafeInteger(version) || version < 1 || version > 2147483647) throw new Error('Invalid version');
